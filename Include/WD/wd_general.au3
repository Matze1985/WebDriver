...
; Related .......:
; Link ..........:
; Example .......: _WD_WaitForSource($sFuncName, '', 'Auto', 5, 1000, True, True)
; ===============================================================================================================================
   ...
   If $bVisible = True Then
      Do
         ...
         If StringRegExp($sSource, $sSearch) Then
            $i = Int($iRound)
         Else
            _WD_HttpResult($sFuncName, "Source", $sSearch & " : is already not visible", 3)
      ...
      Until $i = Int($iRound)
      If StringRegExp($sSource, $sSearch) Then
         _WD_HttpResult($sFuncName, "Source", $sSearch & " : passed", 3)
      Else
   ...
   If $bVisible = False Then
      Do
         ...
         If Not StringRegExp($sSource, $sSearch) Then
            $i = $iRound
         Else
            _WD_HttpResult($sFuncName, "Source", $sSearch & " : is already visible", 3)
            If $bRefresh == True Then
               _WD_BrowserAction($sFuncName, "refresh")
            EndIf
            $i = $i + 1
...
;				   $sActionInput - Value input, [first $sAction and then $sActionInput]
...
; Related .......:
; Link ..........:
; Example .......: _WD_WaitForElementAndAction($sFuncName, "", "//*[@class='btn']", "click", "", 500, 1500)
; ===============================================================================================================================
Func _WD_WaitForElementAndAction($sFuncName, $i_sFrame, $sId, $sAction, $sActionInput, $iDelay, $iTimeout)
...
; Parameters.....: $sFuncName	- Func name for logging
;				   $i_sTab		- Tab (Int) number of all tabs [1 = Current tab] or with strings 'NewTab, CloseTab'
; Author ........: Matze1985
; Modified ......:
; Remarks .......:
; Related .......:
...
Func _WD_TabHandle($sFuncName, $i_sTab)
   ...
   If $i_sTab == 'NewTab' Then
      _WD_NewTab($sSession)
      _WD_HttpResult($sFuncName, "Tab", "New", 0)
   EndIf

   If $i_sTab == 'CloseTab' Then
...
Func _WD_GetAndVerifyElementByValue($sFuncName, $sMethod, $sId, $iNumber, $sProperty, $sVisibleValue, $bVisible, $iWait)
   ...
   Local $sNoHtmlResult = StringRegExpReplace($sResult, "<[^>]*>", "")
...
; Link ..........:
; Examples ......: => Check value for true/false:
;				   _WD_GetAndVerifyElementByText($sFuncName, "1, "//a[@class='tu']", "Hello", True, 2000)
; 				   => Return text:
;				   _WD_GetAndVerifyElementByText($sFuncName, 1, "//a[@class='tu']", "Hello", "", 2000)
; ===============================================================================================================================
Func _WD_GetAndVerifyElementByText($sFuncName, $i_sFrame, $sId, $sVisibleValue, $bVisible, $iWait)
   ...
   $sElement = _WD_FindElement($sSession, $_WD_LOCATOR_ByXPath, $sId)
   Local $sResult = _WD_ElementAction($sSession, $sElement, 'text')
...
; Parameters.....: $sFuncName	- Func name for logging
;				   $sAction		- forward, back, refresh, url
; Author ........: Matze1985
; Modified ......:
; Remarks .......:
; Related .......:
...
; Syntax ........: _WD_ReturnByText($Session, $sFuncName, $sId, $sText, $sEnterFrame, $iWait)
; Parameters ....: $sSession            - Session ID from _WDCreateSession
;				   $sFuncName           - Func name for logging
;				   $i_sFrame			- Frame by int or string (xPath, number, Leave)
;				   $sId					- Method xPath
...
; Related .......:
; Link ..........:
; Examples ......: _WD_ReturnByText($sSession, $sFuncName, 1, "//*[@id='tkl']", "Huhu", 1000)
; ===============================================================================================================================
Func _WD_ReturnByText($sFuncName, $i_sFrame, $sId, $sText, $iWait)
   Local $sElement
   Sleep(Int($iWait))
   If $i_sFrame <> "" Then
      _WD_Frame($sFuncName, $i_sFrame)
   ...
   If StringRegExp($sResult, $sText) Then
      _WD_HttpResult($sFuncName, "ID : " & $sId, "Actual => [" & $sResult & "] Return True => [" & $sText & "]", 2)
      Return True
   Else
      _WD_HttpResult($sFuncName, "ID : " & $sId, "Actual => [" & $sResult & "] Return False => [" & $sText & "]", 2)
      Return False
   EndIf
EndFunc
...
Func _WD_HttpResult($sFuncName, $sDescription, $sLogAction, $iMode = 0)
   ...
   If $iMode == 2 Then
      If $_WD_HTTPRESULT <> 200 Then
         ConsoleWrite($sError & @YEAR & "-" & @MON & "-" & @MDAY & " " & @HOUR & ":" & @MIN & ":" & @SEC & " : " & $sFuncName & " : " & $sDescription & " : " & $sLogAction & " : StatusCode=" & $_WD_HTTPRESULT & @CRLF)
      Else
         ConsoleWrite($sInfo & @YEAR & "-" & @MON & "-" & @MDAY & " " & @HOUR & ":" & @MIN & ":" & @SEC & " : " & $sFuncName & " : " & $sDescription & " : " & $sLogAction & " : StatusCode=" & $_WD_HTTPRESULT & @CRLF)
      EndIf
   EndIf

   ; Mode 3 only for log (StatusCode only by error)
   If $iMode == 3 Then
      If $_WD_HTTPRESULT <> 200 Then
         ConsoleWrite($sError & @YEAR & "-" & @MON & "-" & @MDAY & " " & @HOUR & ":" & @MIN & ":" & @SEC & " : " & $sFuncName & " : " & $sDescription & " : " & $sLogAction & " : StatusCode=" & $_WD_HTTPRESULT & @CRLF)
      Else
